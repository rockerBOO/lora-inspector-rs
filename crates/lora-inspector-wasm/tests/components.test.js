...
test("drag and drop feature detection", (t) => {
	...
	if (isNodeEnvironment) {
		...
		t.true(
			...
			"FileReader not available in Node environment",
		);
	} else {
		// In browser environment, FileReader should be available
		t.true(
			typeof FileReader !== "undefined",
			"FileReader should be available in browser",
		);
	}
});
...
test("date formatting for modelspec", (t) => {
	...
	t.false(Number.isNaN(dateObj.getTime()));
});
...
test("error message state management", (t) => {
	...
	const errorState = createErrorState(
		"Invalid filetype. Try a .safetensors file.",
	);
