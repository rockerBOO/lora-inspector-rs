...
describe("Core UI Components", () => {
	describe("MetaAttribute Component", () => {
		...
		it("should handle long values properly", async () => {
			...
			const longValue =
				"This is a very long value that might wrap or need special handling in the UI component";
	...
	describe("Header Component", () => {
		it("should render with complete metadata", async () => {
			const { Header } = await import(
				"../assets/js/components/layout/Header.jsx"
			);
		...
		it("should handle minimal metadata gracefully", async () => {
			...
			const minimalMetadata = new Map([["modelspec.title", "Basic Model"]]);
...
describe("Metadata Display Components", () => {
	...
	describe("VAE Component", () => {
		it("should display VAE information when available", async () => {
			...
			expect(
				screen.getByText("vae-ft-mse-840000-ema-pruned.ckpt"),
			).toBeDefined();
		});
		...
		it("should handle missing VAE data", async () => {
			...
			const noVaeMetadata = new Map([["modelspec.title", "Model without VAE"]]);
