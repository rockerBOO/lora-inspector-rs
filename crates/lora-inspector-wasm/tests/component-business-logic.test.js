...
test("LoRA file metadata processing pipeline", (t) => {
	...
	const processLoRAMetadata = (rawMetadata) => {
		...
		if (metadata.has("ss_network_dim")) {
			result.networkInfo.dim = parseInt(metadata.get("ss_network_dim"));
		}
		if (metadata.has("ss_network_alpha")) {
			result.networkInfo.alpha = parseFloat(metadata.get("ss_network_alpha"));
		}
		
		// Training processing
		if (metadata.has("ss_training_started_at") && metadata.has("ss_training_finished_at")) {
			const start = parseInt(metadata.get("ss_training_started_at"));
			const end = parseInt(metadata.get("ss_training_finished_at"));
			result.trainingInfo.duration = (end - start) / 60; // minutes
		}
		
		// Dataset processing
		if (metadata.has("ss_num_train_images")) {
			result.datasetInfo.imageCount = parseInt(metadata.get("ss_num_train_images"));
		}
...
test("network component rendering decisions", (t) => {
	...
	const decideNetworkComponents = (metadata) => {
		...
		const supportsDoRA = ["LoRA", "LoHa", "LoRAFA", "LoKr", "GLoRA"].includes(networkType);
		if (supportsDoRA) {
	...
	// Test LoRA network
	const loraMetadata = new Map([["ss_network_type", "LoRA"], ["ss_network_args", "{}"]]);
...
test("dataset component data processing", (t) => {
	...
	const processDatasetInfo = (metadata) => {
		...
		if (metadata.has("ss_num_train_images")) {
			result.totalImages = parseInt(metadata.get("ss_num_train_images")) || 0;
		}
...
test("component conditional rendering logic", (t) => {
	...
	const shouldRenderComponent = (componentName, metadata) => {
		switch (componentName) {
			...
			case "CaptionDropout":
				return metadata.has("ss_caption_dropout_rate") || 
				       metadata.has("ss_caption_tag_dropout_rate");
			
			case "Dataset":
				return metadata.has("ss_datasets") || metadata.has("ss_num_train_images");
